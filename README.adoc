...
To install the CLI (Unix systems only), change to the `zhetapi` directory and run `. ./install` (run `chmod +x install` if necessary). The command will build the CLI and make the `zhetapi` command globally available.
