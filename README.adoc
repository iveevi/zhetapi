...
In addition to being able to defined and use functions, the user can generate
...
generated functions are not modified in any way. Thus, the user it responsible
...
The library already has the ability to generate, compile and link `Function`
objects in runtime. However, as mentioned, the source generated uses the Zhetapi
API. This is disadvantegous in that it takes much longer to compile than native
...
any of the headers in the `inc/std` directory (for exmaple, if the function uses
