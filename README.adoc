...
To install the CLI (Unix systems only), change to the `zhetapi` directory and run `. ./install.sh` (run `chmod +x install.sh` if necessary). The command will build the CLI and make the `zhetapi` command globally available.
